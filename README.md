...
$$X_k = W_k^T X _{k-1}+ b_k, for\ k \in (1,H)$$
...
$$output_{00} = input_{00} * kernel_{00} + input_{01} + kernel_{01} + input_{10} * kernel_{10} + input_{11} * kernel_{11}$$
More generally
$$O_{mn} = \sum_i \sum_j I_{m+i,n+j} K_{ij}$$
...
$$ V_\pi(s) = E_\pi[\sum_{k=0}^\infty R_{t+k+1} | S_t = s], for\ all\ s \in S$$
